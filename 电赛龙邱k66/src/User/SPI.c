...
uint32 spi_init(SPIn_e spin, SPI_PCSn_e pcs, SPI_CFG master,uint32 baud)
{
    ...
    {
//#if defined(MK60DZ10)
        SIM_SCGC6 |= SIM_SCGC6_SPI1_MASK;   //不同的单片机这个寄存器可能略有区别，找到  SCGC6 Bit Fields  中的定义替换即可
...
SPI_CLK_EXIT:
        fit_clk =  bus_clk_M *1000 /(Scaler[fit_br] * Prescaler[fit_pbr]);

        //需要算一下 CS 到 CLK 的时间（波特率的半周期）,满足条件： (1<<(CSSCK + 1 ))*( 2*PCSSCK+1) = fclk /(2*  baud)
        clk =  bus_clk_M*1000/fit_clk/2;
        min_diff = ~0;
        fit_cssck = 0x0F;
        fit_pcssck = 3;
