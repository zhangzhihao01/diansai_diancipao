...
#define TFTSPI_CS     PTE8_OUT    //CS
#define TFTSPI_SCL    PTE9_OUT    //SCL
#define TFTSPI_SDA    PTE10_OUT    //SDI
#define TFTSPI_DC     PTE11_OUT    //DC
#define TFTSPI_RST    PTE12_OUT    //RST
  
 
void TFTSPI_Init(void)
{ 	
  //-----端口初始化----//
  GPIO_Init(PTE,8,GPO,0);
  GPIO_Init(PTE,9,GPO,0);
  GPIO_Init(PTE,10,GPO,0);
  GPIO_Init(PTE,11,GPO,0);
  GPIO_Init(PTE,12,GPO,0);  
...
{ 
  ...
  while(Del--)
  {	
    for(i=0;i<10;i++) asm("NOP");//RT1052 528M: 30000是1ms,30是1M
  }
}
...
{     
  "北京龙邱丘智能科技淘宝店液晶单片机智能车器人启动中电压量不足请稍候，！：℃攻击娱乐模式普通格斗对话俯卧撑倒立握手敬礼跳舞蹈运动蓝精灵太极街舞温度—爱情转移"
};
unsigned char hanzi[] = {	
...
unsigned char hanzi16x16[] = {	
  ...
  0x00,0x0C,0x01,0xFE,0x7F,0x18,0x33,0x18,0x19,0xB0,0x7F,0xFF,0x63,0x03,0xC3,0x06,
  0x7F,0xFC,0x06,0x00,0x07,0xF8,0x0F,0x18,0x19,0xB0,0x30,0xE0,0x63,0xB8,0x1E,0x0F,/*"爱",0*/  //粗体
 
  0x30,0xC0,0x30,0xC0,0x3F,0xFC,0x30,0xC0,0x3F,0xF8,0xFC,0xC0,0xFF,0xFE,0xF0,0x00,
  0xB7,0xF8,0x36,0x18,0x37,0xF8,0x36,0x18,0x37,0xF8,0x36,0x18,0x36,0x78,0x36,0x30,/*"情",1*/

  0x30,0x30,0x30,0x30,0x30,0x30,0xFF,0xFE,0x60,0x30,0x78,0x60,0xDB,0xFF,0xFE,0x60,
  0x18,0xC0,0x19,0xFE,0x1E,0x06,0xF8,0xCC,0x78,0x78,0x18,0x30,0x18,0x18,0x18,0x18,/*"转",2*/

  0x0C,0x30,0x1E,0x30,0xF8,0x7E,0x18,0xC6,0x19,0xEC,0xFE,0x38,0x18,0x30,0x38,0x6C,
  0x3D,0xD8,0x7E,0x3F,0x7E,0x63,0xD9,0xF6,0x18,0x1C,0x18,0x18,0x18,0x70,0x19,0xC0,/*"移",3*/  
};
...
//num: 0 - 65535	要显示的数字
//num_bit: 0 - 5	数字的位数
void TFTSPI_P6X8NUM(uint8_t x, uint8_t y, int num, uint8_t num_bit,uint16_t word_color,uint16_t back_color)
{
    if(num>0)
        {
            num=num;
            TFTSPI_Draw_Line(6*x-4,8*y+4,6*x-1,8*y+4,u16BLACK);
        }
    if(num<0)
        {
            num=-num;
            TFTSPI_Draw_Line(6*x-4,8*y+4,6*x-1,8*y+4,u16WHITE);
        }
